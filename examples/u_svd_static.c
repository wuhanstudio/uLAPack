/**
 * @name svd.c
 * An example of using uLAPAck for SVD matric decomposition.
 *
...
static void run_svd(double Amatrix[17][15]) 
{
    ...
    ulapack_init(&A, 17, 15);
    ulapack_init(&U, 17, 15);
    ulapack_init(&S, 15, 1);
    ulapack_init(&V, 15, 15);

    /*
     * Copy data points into vector objects.
     */
    for (Index_t row_itor = 0; row_itor < 17; row_itor++) {
        for (Index_t col_itor = 0; col_itor < 15; col_itor++) {
...
static void ulapack_svd_thread_entry(void* parameters) 
{
    clock_t start, end;
    float cpu_time_used;
    start = clock();
...
static void ulapack_svd_example(int argc, char *argv[])
{
    rt_thread_t thread = rt_thread_create("u_svd", ulapack_svd_thread_entry, RT_NULL, 20480, 25, 10);
    if(thread != RT_NULL)
    {
        rt_thread_startup(thread);
        rt_kprintf("[ulapack] New thread svd\n");
    }
    else
    {
        rt_kprintf("[ulapack] Failed to create thread svd\n");
    }
}
MSH_CMD_EXPORT(ulapack_svd_example, ulapack SVD example);
